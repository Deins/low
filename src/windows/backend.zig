...
pub const Context = struct {
    ...
    pub fn backendKind(_: *Context) BackendKind { return .windows; }
    pub fn nativeDisplay(_: *Context) *anyopaque { return @ptrCast(win32.GetModuleHandleW(null)); }
    pub fn requiredVulkanInstanceExtensions(_: *Context) []const [*:0]const u8 { return &.{ "VK_KHR_surface", "VK_KHR_win32_surface" }; }
    ...
    pub fn pollEvents(_: *Context) void { _ = dispatchMessages(false, 0); }
    pub fn waitEvents(_: *Context) Error!void { dispatchMessages(true, std.math.maxInt(u32)); }
    ...
    pub fn wake(_: *Context) void { _ = win32.PostMessageW(null, win32.WM_NULL, 0, 0); }
...
pub const Window = struct {
    ...
    pub fn nativeSurface(self: *Window) usize { return @intFromPtr(self.hwnd.?); }
    pub fn nativeDisplay(_: *Window) *anyopaque { return @ptrCast(win32.GetModuleHandleW(null)); }
    pub fn setUserData(self: *Window, ptr: ?*anyopaque) void { self.user_data = ptr; }
    pub fn getUserData(self: *Window) ?*anyopaque { return self.user_data; }
    pub fn setTitle(self: *Window, title: [:0]const u8) void { const wide = std.unicode.utf8ToUtf16LeAllocZ(self.ctx.allocator, title) catch return; defer self.ctx.allocator.free(wide); _ = win32.SetWindowTextW(self.hwnd, wide); }
    pub fn setState(self: *Window, state: WindowState) void { switch (state) { .normal => self.restore(), .maximize => self.maximize(), .fullscreen => self.setFullscreen() } }
    pub fn setShouldClose(self: *Window, value: bool) void { self.should_close = value; }
    pub fn shouldClose(self: *const Window) bool { return self.should_close; }
    pub fn getSize(self: *const Window) Size { return self.size; }
    pub fn getFramebufferSize(self: *const Window) Size { return self.framebuffer_size; }
    pub fn getContentScale(self: *const Window) ContentScale { return self.content_scale; }
    pub fn getCursorPos(self: *const Window) Point { return self.cursor_pos; }
    pub fn isFocused(self: *const Window) bool { return self.focused; }
    pub fn isVisible(self: *const Window) bool { return self.visible; }
    pub fn isMaximized(self: *const Window) bool { return self.maximized; }
    pub fn isFullscreen(self: *const Window) bool { return self.fullscreen; }
    pub fn isIconified(self: *const Window) bool { return self.minimized; }
    pub fn isHovered(self: *const Window) bool { return self.hovered; }
    pub fn getKey(self: *const Window, key: Key) bool { return self.pressed_keys.contains(key); }
    pub fn getMouseButton(self: *const Window, button: MouseButton) bool { return self.pressed_buttons.contains(button); }
    pub fn show(self: *Window) void { _ = win32.ShowWindow(self.hwnd, .{ .SHOWNORMAL = 1 }); self.visible = true; }
    pub fn hide(self: *Window) void { _ = win32.ShowWindow(self.hwnd, .{ .HIDE = 1 }); self.visible = false; }
    pub fn maximize(self: *Window) void { _ = win32.ShowWindow(self.hwnd, @bitCast(@as(u32, 3))); self.maximized = true; self.fullscreen = false; }
    pub fn setFullscreen(self: *Window) void { self.fullscreen = true; self.maximized = false; }
    pub fn restore(self: *Window) void { _ = win32.ShowWindow(self.hwnd, @bitCast(@as(u32, 9))); self.maximized = false; self.fullscreen = false; self.minimized = false; }
    pub fn iconify(self: *Window) void { _ = win32.ShowWindow(self.hwnd, @bitCast(@as(u32, 6))); self.minimized = true; }
    pub fn setMinSize(self: *Window, size: ?Size) void { self.min_size = size; }
    pub fn setMaxSize(self: *Window, size: ?Size) void { self.max_size = size; }
    pub fn setResizable(self: *Window, resizable: bool) void { self.resizable = resizable; }
    pub fn setCursorVisible(self: *Window, visible: bool) void { self.cursor_visible = visible; _ = win32.ShowCursor(if (visible) win32.TRUE else win32.FALSE); }
    pub fn setCursor(self: *Window, shape: CursorShape) void { self.cursor_shape = shape; self.setCursorVisible(shape != .hidden); }
};
...
fn dispatchMessages(wait: bool, timeout_ms: u32) bool {
    ...
    while (win32.PeekMessageW(&message, null, 0, 0, win32.PM_REMOVE) != 0) { had_events = true; _ = win32.TranslateMessage(&message); _ = win32.DispatchMessageW(&message); }
    return had_events;
}

fn windowFromHwnd(hwnd: win32.HWND) ?*Window { const value: usize = @bitCast(win32.GetWindowLongPtrW(hwnd, win32.WINDOW_LONG_PTR_INDEX._USERDATA)); return if (value == 0) null else @ptrFromInt(value); }

fn wndProc(hwnd: win32.HWND, message: u32, wparam: win32.WPARAM, lparam: win32.LPARAM) callconv(.winapi) win32.LRESULT {
    if (message == win32.WM_NCCREATE) { const cs: *win32.CREATESTRUCTW = @ptrFromInt(@as(usize, @bitCast(lparam))); const window: *Window = @ptrCast(@alignCast(cs.lpCreateParams)); _ = win32.SetWindowLongPtrW(hwnd, win32.WINDOW_LONG_PTR_INDEX._USERDATA, @bitCast(@intFromPtr(window))); window.hwnd = hwnd; return win32.DefWindowProcW(hwnd, message, wparam, lparam); }
    const window = windowFromHwnd(hwnd) orelse return win32.DefWindowProcW(hwnd, message, wparam, lparam);
    switch (message) {
        win32.WM_CLOSE => { window.should_close = true; if (window.callbacks.close) |cb| cb(window); return 0; },
        win32.WM_SETFOCUS => { window.focused = true; if (window.callbacks.focus) |cb| cb(window, true); },
        win32.WM_KILLFOCUS => { window.focused = false; if (window.callbacks.focus) |cb| cb(window, false); },
        win32.WM_MOUSEMOVE => { const p = Point{ .x = @floatFromInt(win32.xFromLparam(lparam)), .y = @floatFromInt(win32.yFromLparam(lparam)) }; window.cursor_pos = p; if (window.callbacks.cursor_motion) |cb| cb(window, p); },
        win32.WM_LBUTTONDOWN, win32.WM_LBUTTONUP, win32.WM_RBUTTONDOWN, win32.WM_RBUTTONUP, win32.WM_MBUTTONDOWN, win32.WM_MBUTTONUP, win32.WM_XBUTTONDOWN, win32.WM_XBUTTONUP => {
            ...
            switch (action) { .press => _ = window.pressed_buttons.insert(button), .release => _ = window.pressed_buttons.remove(button), .repeat => {} }
        ...
        win32.WM_KEYDOWN, win32.WM_SYSKEYDOWN, win32.WM_KEYUP, win32.WM_SYSKEYUP => {
            ...
            switch (action) { .press => _ = window.pressed_keys.insert(key), .release => _ = window.pressed_keys.remove(key), .repeat => {} }
...
fn virtualKeyToKey(vkey: win32.VIRTUAL_KEY) Key {
    return switch (vkey) {
        ...
        .RETURN => .enter, .ESCAPE => .escape, .TAB => .tab, .BACK => .backspace, .SPACE => .space,
        .LEFT => .left, .RIGHT => .right, .UP => .up, .DOWN => .down, .HOME => .home, .END => .end, .PRIOR => .page_up, .NEXT => .page_down, .INSERT => .insert, .DELETE => .delete,
        .SHIFT, .LSHIFT => .left_shift, .RSHIFT => .right_shift, .CONTROL, .LCONTROL => .left_control, .RCONTROL => .right_control, .LMENU => .left_alt, .RMENU => .right_alt,
        .LWIN => .left_command, .RWIN => .right_command, .F1 => .f1, .F2 => .f2, .F3 => .f3, .F4 => .f4, .F5 => .f5, .F6 => .f6, .F7 => .f7, .F8 => .f8, .F9 => .f9, .F10 => .f10, .F11 => .f11, .F12 => .f12,
        .OEM_MINUS => .minus, .OEM_PLUS => .equal, .OEM_1 => .semicolon, .OEM_2 => .slash, .OEM_3 => .grave, .OEM_4 => .left_bracket, .OEM_5 => .backslash, .OEM_6 => .right_bracket, .OEM_7 => .apostrophe,
        else => .unknown,
    };
}
